...
void getAtcommand()
{
  ...
  do
  {
    serial_line = BTSerial.readStringUntil('\r\n');
  } while (serial_line == "");
  ...
  */
  ...
  else if (command == "E")
  {
    ...
    BTSerial.write("Exiting debug mode!\r\n");
  ...
  else if (command == "I")
  {
    ...
    // BTSerial.write("AT\r");
    // gsmReadOK();
    // BTSerial.write("AT+CSQ\r");
    // gsmReadOK();
    // getCSQval(readGSMResponse());
    BTSerial.write("AT+CNMI=1,2,0,0,0\r");
  ...
  {
    char logVer[10];
    BTSerial.write("Logger version:");
    itoa(get_logger_version(), logVer, 10);
    BTSerial.write(logVer);  
    BTSerial.write("\r\n");  
  ...
  else if (command == "N")
  {
    ...
    { // 2; 6; 7
      BTSerial.write("Gateway sensor name: ");
      // BTSerial.write(get_logger_A_from_flashMem());
      BTSerial.write(loggerName.sensorA);
      BTSerial.write("\r\n");
    }
    if (isChangeParam())
      inputLoggerNames();
  ...
  {
    //do something
    BTSerial.write("Current command: ");
    sensCommand = passCommand.read();
    BTSerial.write(sensCommand.senslopeCommand);
    BTSerial.write("\r\n");
...
void printMenu()
{
  ...
  //Bluetooth
  BTSerial.write("-----------------------------------------------\r\n");
  BTSerial.write("[A] Sample sensor data\r\n");
  BTSerial.write("[B] Read RTC temperature.\r\n");
  BTSerial.write("[C] Print this menu\r\n");
  BTSerial.write("[D] Change sending time.\r\n");
  BTSerial.write("[E] Exit Debug mode.\r\n");
  BTSerial.write("[F] Change 'server number' from flash memory\r\n");
  BTSerial.write("[G] Print input voltage\r\n");
  BTSerial.write("[H] Reset MCU programmatically\r\n");
  BTSerial.write("[I] GSM receive SMS test\r\n");
  BTSerial.write("[J] Change logger version from flash memory\r\n");
  BTSerial.write("[K] Change MCU password\r\n");
  BTSerial.write("[L] Manual GSM commands\r\n");
  BTSerial.write("[M] Special sending rain data via LoRa\r\n");
  BTSerial.write("[N] Change datalogger names from memory.\r\n");
  BTSerial.write("[O] Read GSM CSQ.\r\n");
  BTSerial.write("[P] Read rain gauge tip.\r\n");
  BTSerial.write("[Q] Reset rain tips.\r\n");
  BTSerial.write("[R] Reset GSM\r\n");
  BTSerial.write("[S] Set date and time.\r\n");
  BTSerial.write("[T] Parse voltage from remote logger.\r\n");
  BTSerial.write("[U] Send rain tips.\r\n");
  BTSerial.write("[V] Sleep GSM\r\n");
  BTSerial.write("[W] Wake GSM\r\n");
  BTSerial.write("[X] wait for Lora data\r\n");
  BTSerial.write("[Y] Send SMS\r\n");
  BTSerial.write("[Z] Change SENSLOPE command.\r\n");
  BTSerial.write("-----------------------------------------------\r\n");
}
...
void setLoggerVersion()
{
  int version;
  char verlog[10];
  BTSerial.write("[0] Sendng data using GSM only\r\n");                //arQ like function only
  BTSerial.write("[1] Version 5 datalogger LoRa with GSM\r\n");         //arQ + LoRa rx
  BTSerial.write("[2] LoRa transmitter for version 5 datalogger\r\n");  //TX LoRa
  BTSerial.write("[3] Gateway Mode with only ONE LoRa transmitter\r\n"); 
  BTSerial.write("[4] Gateway Mode with TWO LoRa transmitter\r\n"); 
  BTSerial.write("[5] Gateway Mode with THREE LoRa transmitter\r\n"); 
  BTSerial.write("[6] LoRa transmitter for Raspberry Pi\r\n");  //TX LoRa
  BTSerial.write("[7] Sends rain gauge data via LoRa\r\n");     //TX LoRa
  BTSerial.write("[8] LoRa dummy transmitter\r\n");     //TX LoRa
  BTSerial.write("[9] LoRa receiver only!\r\n");     //TX LoRa
  delay(1000);
  ...
  {
    BTSerial.setTimeout(8000);
    version = BTSerial.parseInt();
    BTSerial.write("INPUT = ");
    itoa(version, verlog, 10);
    BTSerial.write(verlog);
    BTSerial.write("\r\n");
  }
  delay(50);
...
{
  char commando[15];
  BTSerial.setTimeout(15000);
  BTSerial.write("Insert DUE command: ");
  String dynaCommand = BTSerial.readStringUntil('\r\n');
  Serial.println(dynaCommand);
  dynaCommand.toCharArray(commando, 15);
  BTSerial.write(commando);
  BTSerial.write("\r\n");
...
void inputLoggerNames()
{
  ...
  {
    BTSerial.write("Input name of SENSOR: ");
    String inputLoggerA = BTSerial.readStringUntil('\r\n');
    inputLoggerA.toCharArray(logA, 15);
    BTSerial.write(logA);
    BTSerial.write("\r\n");
    inputLoggerA.toCharArray(loggerName.sensorA, 10);
    flashLoggerName.write(loggerName);
...
bool isChangeParam()
{
  ...
  BTSerial.write("Enter C to change:\r\n");
  do
  {
    serial_line = BTSerial.readStringUntil('\r\n');
  ...
  {
    printMenu();
    BTSerial.write("Change cancelled. Reselect.");
    return false;
  }
}
