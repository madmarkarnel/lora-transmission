...
void send_thru_gsm(char *inputMessage, String serverNumber)
{
  ...
  BTSerial.write("AT+CMGF=1\r");
  gsmReadOK();

  for (int i = 0; i < 3; i++)
  {
    BTSerialFlush();
    BTSerial.write(atCmgsNo); //AT+CMGS="639XXXXXXXXX"\r
    delay(500);
    if (strstr(readGSMResponse(), ">"))
    {
      BTSerial.write(msgToSend);
      delay(500);
      BTSerial.write(26);
    ...
    else
    {
      ...
      BTSerial.write(26);  //need to clear pending gsm cmd
      delay(500);
    }
  }
}
...
char *readGSMResponse()
{
  ...
  for (int i = 0; BTSerial.available() > 0 && i < length; i++)
  {
    response[i] = BTSerial.read();
  }
  return response;
}
...
void gsmHangup()
{
  ...
  delay(2000);
  BTSerial.write("ATH\r");
}

void gsmDeleteReadSmsInbox()
{
  BTSerial.write("AT+CMGD=1,2\r");
  Serial.println(readGSMResponse());
}
...
void resetGSM()
{
  ...
  do
  {
    BTSerial.write("AT\r");  //gsm initialization
...
void turn_ON_GSM()
{
  ...
  BTSerial.write("ATE0\r");  //turn off echo
  gsmReadOK();
  do
  {
...
/*
String readBTSerial(int maxResponseTime = 100)
{
  String response;
  int timeout = 0;
  while (!BTSerial.available() && timeout < maxResponseTime)
  {
    delay(10);
    timeout++;
  }
